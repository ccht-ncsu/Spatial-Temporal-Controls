...
def start_up():
   ...
   if argv[0] in ("-h", "--help"):
      print("Usage:\n  Positional: script.py FORT26_DATE [PRINT_GLOBAL] [TIMESTEP] [SPEC_DIM]\n  " \
         "Flags: script.py --start=DATE --global_fort26=true --increment=1800 --spec_dim=2D")
      sys.exit(0)
      ...
      if not argv:
         ...
         if not argv[0].startswith("--"):
            ...
            for token in argv:
               ...
               elif key in ("fort26_date", "date", "start"):
                  try:
                     FORT26_DATE = float(v)
                  except Exception:
                     raise SystemExit(f"Invalid fort26_date: {v}")
               elif key in ("fort26_timestep", "timestep", "time_step", "increment"):
                  try:
...
# =============================================================
# Find the closest node to a station and report its PE assignment
# =============================================================
def get_closest_node_info(
    station_lon: float,
    station_lat: float,
    nodes: List[Node],
    id_to_node: Dict[int, Node],
    partmesh: List[int],
) -> Tuple[Optional[int], Optional[float], Optional[float], Optional[int], bool]:
    """
    Find the closest mesh node to a station and report its PE.
cd 
    Returns:
        (
            closest_node_id,
            ...
            closest_node_lat,
            closest_node_pe,
            closest_within_thresh,
        )
    """
    node_ids, closest_within_thresh = k_nearest_node_ids_lonlat(
        nodes, station_lon, station_lat, k=1
    )

    if not node_ids:
        return None, None, None, None, False

    closest_node_id = node_ids[0]
    node = id_to_node[closest_node_id]
    closest_node_pe = partmesh[closest_node_id - 1]
...
def pick_by_centroid_lonlat(
   ...
   best_centroid = None
      ...
      # collect (element, centroid_x, centroid_y, d2) for reporting
   candidates: List[Tuple[Element, float, float, float]] = []
   ...
   for e in elements:
      ...
      d2 = (cx - p_lon)**2 + (cy -p_lat)**2
         # record candidate for later reporting
      candidates.append((e, cx, cy, d2))

      if d2 < best_d2:
         best_d2, best = d2, e
         best_centroid = (cx, cy)
      # Print the top-3 closest element centroids (if available)
      if candidates:
         # sort by squared distance
         candidates.sort(key=lambda t: t[3])
         top_n = min(3, len(candidates))
         print(f"    Top {top_n} closest element centroids to point ({p_lon:.6f}, {p_lat:.6f}):")
         for i in range(top_n):
            e_i, cx_i, cy_i, d2_i = candidates[i]
            dist_i = math.sqrt(d2_i)
            print(f"      {i+1}) Element {e_i.id}: centroid=({cx_i:.6f}, {cy_i:.6f}), distance={dist_i:.6f} degrees")
...
def point_in_element(px: float, py: float, 
   ...
   # if not matches:
   #    print(f"    Point ({px:.6f}, {py:.6f}) is NOT inside found element")
   #    print(f"Total area (AT): {AT}")
   #    print(f"Sum of sub-areas (A1+A2+A3): {A1 + A2 + A3}")
   return matches


def inspect_fort18_node_signs(pe_dir: str, element: Element) -> Dict[int, str]:
   """Inspect `PExxxx/fort.18` and report whether each node of `element`
   appears as positive, negative, both, or missing. Also include the
   element id in the same returned dictionary using the key `element.id`.

   Returns a dict mapping node_id (and element.id) -> one of
   {'positive','negative','both','missing'}.
   """
   import re

   out: Dict[int, str] = {}
   fort18_path = os.path.join(pe_dir, "fort.18")
   if not os.path.isfile(fort18_path):
      return {element.id: "missing", element.n1: "missing", element.n2: "missing", element.n3: "missing"}

   try:
      with open(fort18_path, "r", encoding="utf-8", errors="ignore") as fh:
         text = fh.read()
   except Exception:
      return {element.id: "missing", element.n1: "missing", element.n2: "missing", element.n3: "missing"}

   # Find all integer tokens with optional sign
   tokens = re.findall(r'(?<!\d)([-]?\d+)(?!\d)', text)
   ints = [int(t) for t in tokens]
   ints_set = set(ints)

   # Also determine the file line where the element (or its negative) appears.
   # If found, print the line number -2 (two lines before the match) for debugging.
   lines = text.splitlines()
   found_line_idx = None
   for idx, line in enumerate(lines):
      line_tokens = re.findall(r'(?<!\d)([-]?\d+)(?!\d)', line)
      line_ints = [int(t) for t in line_tokens]
      if element.id in line_ints or (-element.id) in line_ints:
         found_line_idx = idx
         break
   if found_line_idx is not None:
      # convert to 1-based line numbers for display; compute line-2 (but not less than 1)
      display_found = found_line_idx + 1
      display_line_minus2 = max(1, found_line_idx - 1)
      print(f"      element {element.id} found in {fort18_path} at line {display_found}; line-2 -> {display_line_minus2}")

   for id in (element.n1, element.n2, element.n3, element.id):
      pos = id in ints_set
      neg = -id in ints_set
      if pos and neg:
         out[id] = "both"
...
def find_element_for_station(
   ...
   Returns:
      (element_or_None, maybe_station_id_for_reporting)
   """
   ...
   with timer(f"find_element_for_station station={station_id}"):
      for k in k_list:
         ...
         with timer(f"   K nearest nodes (k={k})"):
            cand_node_ids, _ = k_nearest_node_ids_lonlat(nodes, station_lon, station_lat, k)                     
         ...
         if inside:            
            ...
            return chosen_element, large_sid
         else:
            print(f"    Element #{chosen_element.id} does NOT hold the station.")
            ...
            # Only try to advance to the next k if there is one available
            if n < len(k_list) - 1:
               ...
               # No larger k available; report and break out to return failure
               print("       Cannot increase k further; reached maximum search radius.")
               break
      return None, None
...
def find_pe_for_station(
      ...
      with timer(f"find_pe_for_station station={station_id}"):
         # Step 1: get closest node info
         closest_node_id, closest_node_lon, closest_node_lat, closest_node_pe, closest_within_thresh = get_closest_node_info(
         ...
         print("============ Closest Node Info ============")
         print(f"    Station ID: {station_id}")
         print(f"    Station Lon/Lat: ({station_lon:.6f}, {station_lat:.6f})")
         if closest_node_id is not None:
            print(f"    Closest Node ID: {closest_node_id}")
            print(f"    Closest Node Lon/Lat: ({closest_node_lon:.6f}, {closest_node_lat:.6f})")
            print(f"    Closest Node PE: PE{closest_node_pe:04d}")
            print(f"    Station-on-node threshold match: {closest_within_thresh}")
            # Case 1: If the closest node is within threshold, assign PE based on that node and return early
            if closest_within_thresh:
               print(
                  f"================ Station on Node ================",
                  f"\n  Station is on/very near node {closest_node_id}",
                  f"\n  Assigning PE {closest_node_pe} based on that node."
               )               
               return closest_node_pe, None
         else:
            print("    No closest node found.")

         # # Step 3: otherwise find containing element         
         element, maybe_sid = find_element_for_station(station_lon, station_lat, nodes, id_to_node, node_to_elements, station_id)

         if not element:
            print(f"WARNING: No containing element found for station {station_id}")            
            if closest_node_id is not None and closest_node_pe is not None:
               print(f"    Falling back to closest node {closest_node_id} -> PE{closest_node_pe:04d}")
               return closest_node_pe, None
            else:
               warnings.warn(f"Could not determine PE for station {station_id}. No nearby node or containing element found.")
               return None, None
         else: 
      ...
      with timer("PE Assignment"):                     
         ...
         print(f"    The element's nodes belong to PE folders: {pe_numbers} (unique: {unique_pes})")

         if len(unique_pes) == 1:
            pe_for_station = pe_numbers[0]
            print(f"    All element nodes are in the same PE -> assigning PE{pe_for_station:04d}")

         else:
            print(f"   WARNING: station {station_id} element {element.id} nodes span multiple PEs: {pe_numbers}")
            print(f"    Assigning station to PE of closest node {closest_node_id} -> PE{closest_node_pe:04d}")
            pe_for_station = closest_node_pe
         ...
         # print("============ PE Comparison ============")
         # print(f"    Element-based PE: PE{pe_for_station:04d}")
         # if closest_node_pe is not None:
         #    print(f"    Closest-node PE:  PE{closest_node_pe:04d}")
         #    if pe_for_station == closest_node_pe:
         #       print("    RESULT: element-based PE and closest-node PE MATCH")
         #    else:
         #       print("    RESULT: element-based PE and closest-node PE DO NOT MATCH")
